...
const Write = () => {
    ...
    const uploadPostNow = async(postObj) => {
      try {
        const response = await axios.post(
          `https://blogging-app-server.herokuapp.com/api/posts/publish`,
          postObj
        ).then(res => {
          navigate('/published');
        })
    ...
    const uploadPostNow = async(postObj) => {
      try {
        const response = await axios.put(
          `https://blogging-app-server.herokuapp.com/api/posts/${location.state.oldPostId}/publish`,
          postObj
        ).then(res => {
          navigate('/published');
        })
  ...
  const draftNewPost = async(postObj) => {
    const uploadPostNow = async(postObj) => {
      try {
        const response = await axios.post(
          `https://blogging-app-server.herokuapp.com/api/posts/draft`,
          postObj
        ).then(res => {
          navigate('/draft');
        })
  ...
  const updateAndDraftOldPost = async(postObj) => {
    const uploadPostNow = async(postObj) => {
      try {
        const response = await axios.put(
          `https://blogging-app-server.herokuapp.com/api/posts/${location.state.oldPostId}/draft`,
          postObj
        ).then(res => {
          navigate('/draft');
        })
    ...
    <div>
      <Box style={{marginBottom: 30, paddingLeft: 0, overflowWrap: 'break-word', wordWrap: 'break-word'}}>
        <Button 
          ...
          style={{width: 160, color: '#ffff', background: 'rgb(26,136,22)', textTransform: 'none', borderRadius: 100, paddingTop: 8, paddingBottom: 8}}
      ...
      <Box style={{ paddingTop: 0, paddingLeft: 0}}>
        <Button 
          ...
          style={{width: 160, color: '#fff', background: 'rgb(133, 133, 133)', textTransform: 'none', borderRadius: 100, paddingTop: 8, paddingBottom: 8}}
