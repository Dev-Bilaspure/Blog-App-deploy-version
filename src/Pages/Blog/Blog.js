...
const Blog = ({user}) => {
  ...
  useEffect(() => {
    const fetchBlogData = async () => {
      try {
        const response = await axios.get(
          `https://blogging-app-server.herokuapp.com/api/posts/${postID}`
        ).then(res => {
          setTitle(res.data.title);
          document.title = `Maadhyam | ${res.data.title}`
    ...
    const fetchAuthorInfo = async(userId) => {
      try {
        const response = await axios.get(
          `https://blogging-app-server.herokuapp.com/api/users/userbyid/${userId}`
        ).then(res => {
    ...
    if(!user)
      ...
      try {
        const response = await axios.put(
          `https://blogging-app-server.herokuapp.com/api/users/${postsUsersId}/follow`,
          {userId: user._id}
        ).then(res => {
  ...
  const handleClickBookmark = async() => {
    ...
    try {
      const response = await axios.put(
        `https://blogging-app-server.herokuapp.com/api/posts/${postID}/bookmark`,
        {userId: user._id}
      ).then(res => {
        loginSuccess(res.data);
        // console.log(res.data);
    ...
    <Box className='blogWrapper'>
      ...
      <Grid container style={{marginTop: 10, paddingTop: 20, paddingRight: 0, paddingLeft: 0}}>
        ...
        <Grid item lg={6} md={6} sm={12} xs={12} style={{width: '100%', paddingRight: 10, paddingLeft: 10}} >
          ...
          <Box style={{marginTop: 25, marginBottom: 40}}>
            <MiddleBanner 
              isFollowing={isFollowing} 
              handleFollowButton={handleFollowButton} 
