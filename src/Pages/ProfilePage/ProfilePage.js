    ...
    const fetchUsersInfo = async () => {
      try {
        const response = await axios.get(
          `https://blogging-app-server.herokuapp.com/api/users/userbyusername/${username}`
        ).then(res => {
          const userkiId = res.data._id;
          setUsersname(res.data.name);
          document.title = `Maadhyam | ${res.data.name}`
    ...
    if(!user)
      ...
      try {
        const response = await axios.put(
          `https://blogging-app-server.herokuapp.com/api/users/${profileUsersId}/follow`,
          {userId: user._id}
        ).then(res => {
...
const PostByUser = ({user, post}) => {
  ...
  const handleClickBookmark = async() => {
    ...
    try {
      const response = await axios.put(
        `https://blogging-app-server.herokuapp.com/api/posts/${post._id}/bookmark`,
        {userId: user._id}
      ).then(res => {
        loginSuccess(res.data);
        console.log(res.data);
  ...
  return(
    <Box className={classes.postByUserWrapper} >
      ...
      <Typography style={{fontSize: 37, fontFamily: `'Signika', 'sans-serif'`, paddingTop: 5, lineHeight: 1.26, color: 'rgb(41,41,41)'}}>
