...
const UsersListItem = ({user, usersListItem}) => {
  const [isFollowing, setIsFollowing] = useState(user && user.followings.indexOf(usersListItem._id)>-1);
  ...
  const handleClickFollow = async() => {
    ...
    try {
      const response = await axios.put(
        `https://blogging-app-server.herokuapp.com/api/users/${usersListItem._id}/follow`,
        {userId: user._id}
      ).then(res => {
          ...
          <Grid container>
            <Grid item>
              <Avatar
                ...
                src={usersListItem.profilePicture ? usersListItem.profilePicture : defaultUserPic}
                style={{height: 40, width: 40, marginTop: 0}}
              /> 
            </Grid>
            <Grid item style={{marginLeft: 10, paddingTop: 0}}>
              <Typography style={{fontSize: 13, color: '#000000'}}>
                <Link to={`/profile/@${usersListItem.username}`} style={{color: 'inherit', textDecoration: 'none'}}>
                  {usersListItem.name}
                </Link>
              </Typography>
