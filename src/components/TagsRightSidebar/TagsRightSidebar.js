    ...
    const fetchRankedUsers = async () => {
      try {
        const response = await axios.get(
          `https://blogging-app-server.herokuapp.com/api/users/ranked`
        ).then(res => {
  ...
  const handleClickFollowBtn = async() => {
    ...
    try {
      const response = await axios.put(
        `https://blogging-app-server.herokuapp.com/api/users/${rankedUser._id}/follow`,
        {userId: user._id}
      ).then(res => {
      ...
      <Grid container>
        <Grid item style={{marginBottom: 10}}>
          <Link to={`/profile/@${rankedUser.username}`} style={{color: 'inherit', textDecoration: 'none'}}>
            <Avatar
              alt="author avatar"
              src={rankedUser.profilePicture ? rankedUser.profilePicture : defaultUserPic}
